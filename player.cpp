...
namespace menu {
	...
	void player_menu::render() {
		renderer::addSubmenu("Player", "Player", [](core* core) {
			...
			core->addOption(toggleOption("Semi Godmode")
				.addHotkey().addTranslate()
				.addToggle(&m_vars.m_semi_godmode));
			...
			core->addOption(toggleOption("Invisible")
				.addHotkey().addTranslate()
				.addToggle(&m_vars.m_invisible).addClick([] {
			...
			core->addOption(toggleOption("Octopus Mode")
				.addHotkey().addTranslate()
				.addToggle(&m_vars.m_Octo_Mode));
	...
	void player_menu::update() {
		...
		if (m_vars.m_Octo_Mode) {
			PED::SET_PED_TO_RAGDOLL(PLAYER::PLAYER_PED_ID(), 10, 10, 0, true, true, true);
			if (Util::is_key_pressed(VK_SPACE)) ApplyForceToEntity(PLAYER::PLAYER_PED_ID(), 0, 2, 0, 0, 0 ,0);
			if (Util::is_key_pressed(VK_SHIFT)) ApplyForceToEntity(PLAYER::PLAYER_PED_ID(), 2, 0, 0, 0, 0, 0);
			if (Util::is_key_pressed(VK_CONTROL)) ApplyForceToEntity(PLAYER::PLAYER_PED_ID(), 0, -2, 0, 0, 0, 0);
		}

		if (m_vars.m_fast_respawn) {
			Vector3 Pos = ENTITY::GET_OFFSET_FROM_ENTITY_IN_WORLD_COORDS(PLAYER::PLAYER_PED_ID(), {0.0f, 2.0f, 0.0f});
			...
			if (PED::IS_PED_DEAD_OR_DYING(PLAYER::PLAYER_PED_ID(), true))
			{
				...
				PED::SET_PED_CONFIG_FLAG(PLAYER::PLAYER_PED_ID(), 73, false);
				MISC::DISABLE_HOSPITAL_RESTART(PLAYER::PLAYER_PED_ID(), true);
				CAM::RENDER_SCRIPT_CAMS(false, false, 0, true, false, false);
				PED::SET_PED_CONFIG_FLAG(PLAYER::PLAYER_PED_ID(), 203, true);
				PHYSICS::SET_USE_KINEMATIC_PHYSICS(PLAYER::PLAYER_PED_ID(), true);
			}
		}
		else
		{
			MISC::DISABLE_HOSPITAL_RESTART(PLAYER::PLAYER_PED_ID(), false);
			CAM::RENDER_SCRIPT_CAMS(false, false, 0, true, false, false);
			PED::SET_PED_CONFIG_FLAG(PLAYER::PLAYER_PED_ID(), 203, false);
