...
namespace menu::player::appearance::vars {
	...
	inline const char* g_ped_models[670] = {
...
namespace menu {
	...
	void player_appearance_menu::render() {
		renderer::addSubmenu("Appearance", "Player Appearance", [](core* core) {
			...
			core->addOption(breakOption("Appearance"));

			core->addOption(scrollOption<const char*, int>("Models")
				.addScroll(&g_ped_models).setPosition(&g_ped_models_id)
				.addClick([=] {
					if (Util::is_key_pressed(VK_RETURN)) {
						apply_model(g_ped_models[g_ped_models_id]);
					}
					}));
